...
class LtSpiceServer(SpiceServer):
    ...
    ##############################################
    def __init__(self, spice_command='scad3.exe'): 
...
def enableLtSpice(simulator, spice_command='"c:\\Program Files (x86)\\LTC\\LTspiceIV\\scad3.exe"'):
    simulator._options.pop('filetype')
    simulator._options.pop('NOINIT')
    simulator._spice_server= LtSpiceServer(spice_command=spice_command)